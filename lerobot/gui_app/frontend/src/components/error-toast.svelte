...
// subscribe to errorMessages store and react whenever a new error is added
const unsubscribe = errorMessages.subscribe((errors) => {
    if (errors.length > 0) {
...
{#if showToast}
<div class="toast show bg-danger-subtle position-fixed bottom-0 end-0 p-2 m-3" role="alert" aria-live="assertive" aria-atomic="true" style="min-width: 250px;">
